...
function Butterfly(textureID){
	...
	this.loadTexture = function() {
		...
		var mt = this.wingsMaterialPhong;
		var ob = this.butterflyMesh;

		this.imageLoader.load('http://'+ipServer+':6001/butterflyTextures/'+ btoa(idString) + '.png', function(image){ 
			tex.image = image;
			tex.needsUpdate = true;
			/*
			// add as say in : http://stackoverflow.com/questions/15137695/three-js-lightmap-causes-an-error-webglrenderingcontext-gl-error-gl-invalid-op
			mt.map = tex;
			ob.material = mt;
	...
	this.wings = function(){
		...
		this.body_top();
		this.body_lower();
		this.wingsMaterialPhong = new THREE.MeshPhongMaterial( { 
		...
		this.butterflyMesh = new ButterflyMesh( this.geometry, this.wingsMaterialPhong );
