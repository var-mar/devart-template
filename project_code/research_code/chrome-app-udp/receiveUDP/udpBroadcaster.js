...
function UDPManager(callback_receive, type, arrayIPs){
  ...
  this.debug = false;
  this.port = 3007;
  this.ipBroadcast = "237.132.123.123";
  ...
  this.send = function (buf){
    if(self.socketId!=undefined){
      if(self.type=='broadcast'){
        self.socket.sendTo(self.socketId, buf, self.ipBroadcast, self.port, function(sendResult) {
          if(self.debug)console.log("sendTo", sendResult);
        });
      }else{
        for(var i=0;i<arrayIPs.length;i++){
          var ip = arrayIPs[i];
          self.socket.sendTo(self.socketId, buf, ip, self.port, function(sendResult) {
            if(self.debug)console.log("sendTo", sendResult);
          });
        }
      }
